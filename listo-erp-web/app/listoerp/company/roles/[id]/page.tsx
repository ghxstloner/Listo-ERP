...
export default function CompanyRoleDetailPage() {
  ...
  const [permissions, loadingPermissions, permissionsError] =
    useGetPermissions();
  if (loadingRoles || loadingPermissions)
    return (
      <PageLoading text="Cargando rol..." icon={<Spinner size={32} />} spin />
    );
  const role = roles?.find((item) => item.id === roleId);
  if (rolesError || permissionsError || !role)
    return (
      <div className="flex min-h-[400px] flex-col items-center justify-center gap-4">
        <p className="text-destructive">No se pudo cargar el rol.</p>
        <Button variant="outline" asChild>
          <Link href="/listoerp/company?tab=roles">Volver a empresa</Link>
        </Button>
      </div>
    );
  return (
    <div className="w-full p-2">
      <Button size="sm" variant="ghost" asChild>
        <Link
          href="/listoerp/company?tab=roles"
      ...
      <div className="mt-2">
        <CompanyRoleForm role={role} permissions={permissions ?? []} />
      </div>
    </div>
  );
}
