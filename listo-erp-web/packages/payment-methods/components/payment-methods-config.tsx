...
function SortableHeader({
  ...
  column: Column<PaymentMethod, unknown>;
...
export function PaymentMethodsConfig() {
  ...
  const save = useMutation({
    mutationFn: (request: PaymentMethodRequest) =>
      editing
        ? api.patch<PaymentMethodResponse>(`payment-methods/${editing.id}`, {
            body: request,
          })
    ...
    onError: (mutationError) =>
      showToast({ type: "error", message: mutationError.message }),
  });
  const remove = useMutation({
    mutationFn: (id: number) =>
      api.delete<{ message: string }>(`payment-methods/${id}`),
    onSuccess: (response) => {
      invalidate();
      showToast({ type: "success", message: response.message });
    },
  ...
  const uploadImage = useMutation({
    mutationFn: ({ id, file }: { id: number; file: File }) =>
      uploadPaymentMethodImage(id, file),
    onSuccess: () => {
      invalidate();
  ...
  const submit = () => {
    if (!form.name.trim() || !form.code.trim()) {
      showToast({
        type: "error",
        message: "El nombre y código son obligatorios.",
      });
      return;
    }
    save.mutate({
      ...
      dianCode: form.dianCode?.trim().toUpperCase() || null,
    });
  };
    ...
    () => [
      {
        ...
        header: ({ column }) => (
          <SortableHeader column={column}>Imagen</SortableHeader>
        ),
        cell: ({ row }) =>
          row.original.image ? (
            <img
              src={getPaymentMethodImageUrl(row.original.image)}
              alt=""
      ...
      },
      {
        ...
        accessorKey: "code",
        header: ({ column }) => (
          <SortableHeader column={column}>Código interno</SortableHeader>
        ),
        cell: ({ row }) => (
      ...
      {
        id: "dianCode",
        accessorFn: (row) => row.dianCode ?? "",
        header: ({ column }) => (
          <SortableHeader column={column}>Código DIAN</SortableHeader>
        ),
        cell: ({ row }) => (
          <span className="font-mono text-xs">
            {row.original.dianCode ?? "-"}
          </span>
        ),
      },
      {
        ...
        cell: ({ row }) => (
          <span
            className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${row.original.isActive ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400" : "bg-muted text-muted-foreground"}`}
          >
        ...
        cell: ({ row }) => (
          <div className="flex justify-end">
            <Button
              ...
              aria-label={`Cambiar imagen de ${row.original.name}`}
            >
  ...
  const handleConfirmDelete = () => {
    ...
    remove.mutate(methodToDelete.id, {
      onSettled: () => setMethodToDelete(null),
    });
  };
  ...
  return (
    <>
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0">
          <div>
            ...
            <CardDescription>
              Administra los métodos de pago de la empresa y sus imágenes.
            </CardDescription>
          </div>
      ...
      >
        <DialogContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="payment-method-code">Código interno</Label>
              <Input
            ...
            <div className="space-y-2">
              <Label htmlFor="payment-method-dian-code">Código DIAN</Label>
              <Input
                id="payment-method-dian-code"
                value={form.dianCode ?? ""}
                onChange={(event) =>
                  setForm({
                    ...form,
                    dianCode: event.target.value.toUpperCase(),
                  })
                }
                placeholder="10"
              />
              <p className="text-xs text-muted-foreground">
                Se utiliza para facturación electrónica en Colombia.
              </p>
            </div>
      ...
      <ConfirmDialog
        ...
        description={`¿Estás seguro de que deseas eliminar el método de pago "${methodToDelete?.name}"? Esta acción no se puede deshacer.`}
